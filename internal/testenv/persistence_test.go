...
func TestBehavior_Persistence_MagnetsAndLinks(t *testing.T) {
	...
	magnet, err := seeder.AddFile("persist.txt", content, tracker.AnnounceURL())
	require.NoError(t, err)
	...
	t.Log("--- STARTING SESSION 1 ---")
	{
		...
		waitForFile(t, app, "/unique-p-route/persist.txt")
	...
	t.Log("--- STARTING SESSION 2 ---")
	{
		...
		waitForFile(t, app, "/unique-p-route/persist.txt")
		data, err := readFile(t, app, "/unique-p-route/persist.txt")
		require.NoError(t, err)
